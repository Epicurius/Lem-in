...
void	print1(t_lem_in *lem)
{
	t_link *current;
	current = lem->links;
	while (current != NULL)
	{
		printf("%s-", current->start->name);
		printf("%s %d\n", current->end->name, current->used);
		//printf("%d ", current->yx[0]);
		//printf("%u\n", current->type);
		current = current->next;
	}
}

void	print(t_lem_in *lem)
{
	t_room *current;
	current = lem->rooms;
	while (current != NULL)
	{
		...
		printf("in:%d ", current->paths[0]);
		printf("out:%d ", current->paths[1]);
		//printf("same:%d ", current->paths[2]);
		printf("bfs = %d\n", current->bfs_val);
		current = current->next;
	}
}

void	print2(t_lem_in *lem)
{
	t_route *current;
	t_link *current1;
	...
	{	
		...
		while (current1)
		{
			printf("[%s]->", current1->start->name);
			if (!current1->next)
				printf("[%s]", current1->end->name);
